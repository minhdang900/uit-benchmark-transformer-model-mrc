...
Không có tìm kiếm siêu tham số theo nghĩa quét có hệ thống: tốc độ học $3\cdot10^{-5}$ và lịch học
là giá trị phổ biến cho mô hình cỡ \emph{base} trên SQuAD, và cả hai mô hình đều bắt đầu từ đó.
Ngân sách sau đó \emph{không} cân bằng, và điều này phải được nói rõ: PhoBERT được chạy thêm ở
$2\cdot10^{-5}$ và $10^{-5}$ để \emph{chẩn đoán} sự kiện sụp đổ (Mục~\ref{sec:res-rq3}), còn XLM-R
chỉ có một tốc độ học. Các lần chạy thêm đó được chọn trên dev, không phải trên validation, nhưng
chúng vẫn là ba lần thử cho một họ mô hình và một lần cho họ kia --- nên phần hơn của PhoBERT về
điểm tổng phải được đọc kèm điều kiện đó. Ngoài ra cả hai mô hình có thể chưa đạt điểm tốt nhất:
mọi lần chạy đều còn đang cải thiện ở epoch cuối.
...
\subsection{Dựng và kiểm định bộ stress-test v2}
\label{sec:audit-method}

Bộ stress-test dùng trong báo cáo này (\emph{v2}) được dựng lại từ \textbf{UIT-ViQuAD 2.0
validation} theo một nguyên tắc: \emph{không tự viết câu hỏi và không tự gán nhãn}. Mọi câu hỏi
và đáp án vàng đều do người gán trong bộ gốc. Mỗi mục thuộc một trong hai loại:

\begin{itemize}
  \item \textbf{slice} --- câu gốc giữ nguyên, được \emph{chọn} vì mang một hiện tượng đo được
        (ví dụ biên đáp án cắt ngang một từ của bộ tách từ; đáp án nằm ngoài cửa sổ token đầu);
  \item \textbf{perturbation} --- câu gốc bị biến đổi bằng một luật mà nhãn mới \emph{suy ra
        được} từ nhãn cũ: xoá câu chứa đáp án $\Rightarrow$ không trả lời được; chèn một câu
        nhiễu không nói về chủ thể của câu hỏi $\Rightarrow$ đáp án không đổi; bỏ dấu tiếng Việt
        trong câu hỏi $\Rightarrow$ đáp án không đổi. Mỗi perturbation đi kèm chính câu gốc của
        nó (vai trò \code{original}) để đo \textbf{độ nhất quán theo cặp}, không chỉ điểm trung bình.
\end{itemize}

Trước khi bộ này được dùng để chấm bất kỳ mô hình nào, hàm \code{mrc.stress\_v2.audit} kiểm các
bất biến sau trên từng mục: (1) với câu có đáp án, đáp án vàng nằm đúng tại \code{answer\_start}
đã ghi trong ngữ cảnh; (2) với câu bị xoá câu chứa đáp án, không đáp án gốc nào --- kể cả sau
chuẩn hoá --- còn sót trong ngữ cảnh; (3) câu hỏi nhóm E5 không còn dấu tiếng Việt; (4) câu nhiễu
chèn thêm không chứa đáp án vàng; (5) mỗi perturbation có câu gốc đi kèm; (6) không ngữ cảnh nào
trùng với tập train. Bộ dữ liệu \textbf{hợp lệ} khi số vi phạm bằng 0 --- nghĩa là \emph{mọi} mục
đều chấm được, khác với việc lọc bớt câu hỏng sau khi đã chấm.

Để bộ trải rộng thay vì dồn vào vài đoạn văn, mỗi tập con lấy tối đa hai mục từ cùng một ngữ
cảnh. Bộ v2 chỉ dùng để \emph{chẩn đoán sau khi mô hình đã chốt}: không epoch, seed hay ngưỡng
$\tau$ nào được chọn trên nó. Mỗi mô hình được chấm ở đúng $\tau$ đã chọn trên dev cho nó
(Mục~\ref{sec:threshold}), áp một lần --- cùng giao thức với validation; điểm từng cửa sổ được lưu
nên dự đoán ở $\tau$ đó dựng lại được mà không chạy lại mô hình, và ở $\tau = 0$ chúng trùng khớp
với lần chấm trực tiếp.

Với ba tập con có cặp (E2c, E3b, E5), con số chính là \textbf{tỉ lệ hỏng}: trong số cặp mà mô
hình trả lời đúng \emph{câu gốc}, tỉ lệ cặp nó trả lời sai \emph{sau biến đổi}. Chỉ số này quy lỗi
cho đúng phép biến đổi; EM trung bình thì trộn lẫn hiệu ứng đó với những câu mô hình vốn đã sai.
Phần bù của nó --- tỉ lệ cặp đúng cả hai --- là độ nhất quán theo cặp; đó là hai cách đọc cùng một
phép đo, không phải hai chỉ số. Kết quả kiểm định ở Mục~\ref{sec:audit}; điểm của các hệ thống ở
Mục~\ref{sec:res-rq3}.
...
nào chưa có sẽ hiện chữ đỏ ``??'' trong PDF thay vì một con số đoán. Mã nguồn có \NTests{} kiểm thử
