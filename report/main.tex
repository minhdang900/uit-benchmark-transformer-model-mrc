...
% listings chỉ chặn ký tự <256; dưới XeTeX, chữ tiếng Việt ngoài Latin-1 được in NGAY
% trong khi listings còn giữ các chữ ASCII của cùng từ trong bộ đệm, nên bị đảo thứ tự
% ("dựng" -> "ựdng"); literate KHÔNG sửa được (đã thử). Cách sửa: đưa chữ tiếng Việt
% qua escapeinside để TeX in trực tiếp. \lstcmt tái tạo commentstyle ở trên cho phần
% chú thích được thoát như vậy — sửa commentstyle thì sửa cả đây.
\newcommand{\lstcmt}[1]{{\color{black!55}\itshape #1}}
...
\input{chapters/04a_du_lieu}
\input{chapters/04b_ket_qua}
\input{chapters/05_ket_luan}
\input{chapters/06_tham_khao}
\input{chapters/07_phu_luc}
