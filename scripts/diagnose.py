...
MODELS = ["abstain", "baseline", "xlmr", "phobert"]
...
def main() -> None:
    ...
    if {"xlmr", "phobert"} <= set(preds):
        report["paired_phobert_vs_xlmr"] = {
            "all": paired_comparison(em["phobert"], em["xlmr"]),
            "answerable": paired_comparison({q: em["phobert"][q] for q in answerable},
                                            {q: em["xlmr"][q] for q in answerable}),
            "impossible": paired_comparison({q: em["phobert"][q] for q in refs if not refs[q]},
                                            {q: em["xlmr"][q] for q in refs if not refs[q]}),
            "misaligned": paired_comparison({q: em["phobert"][q] for q in misaligned},
                                            {q: em["xlmr"][q] for q in misaligned}),
        }
        # Câu PhoBERT sai mà XLM-R đúng, và ngược lại: nguyên liệu cho phân tích định tính.
        disagree = []
        for q in refs:
            if em["phobert"][q] != em["xlmr"][q]:
                disagree.append({
                    "qid": q, "question": by_qid[q].question, "gold": refs[q],
                    "phobert": preds["phobert"].get(q, ""), "xlmr": preds["xlmr"].get(q, ""),
                    "winner": "phobert" if em["phobert"][q] > em["xlmr"][q] else "xlmr",
                    "phobert_error": classify_error(preds["phobert"].get(q, ""), refs[q]),
