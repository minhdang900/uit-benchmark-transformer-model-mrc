...
Kế thừa từ pipeline CS116 của cùng nhóm; khác biệt cho CS221:

* **Chọn epoch trên tập dev tách từ TRAIN** (5% context, chia theo context), không
  phải trên validation. Validation chính thức đóng vai tập TEST và chỉ được chấm
  đúng một lần, bởi ``scripts/run_eval.py``. Test chính thức của ViQuAD 2.0 là
  blind (gold bị lược bỏ) nên không chấm được.
* **``--word-segmented``** cho PhoBERT: tokenizer được bọc bởi
  :class:`mrc.segmented_tokenizer.SegmentedTokenizer` để có ``offset_mapping``.
...
from mrc.data import assert_no_leakage, load_squad_file, split_by_context
from mrc.tokenization import load_tokenizer, write_tokenization_config
...
def main(argv=None) -> None:
    ...
    tokenizer = load_tokenizer(args.model, word_segmented=args.word_segmented)
    ...
    print(f"assert_no_leakage OK — train {len({e.context for e in train_ex})} ctx / "
          f"dev {len({e.context for e in dev_ex})} ctx / "
          f"validation(test) {len({e.context for e in val_ex})} ctx, giao nhau = 0")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        write_tokenization_config(checkpoint, word_segmented=args.word_segmented,
        ...
        predictor = TransformerQA.from_checkpoint(str(checkpoint), device=device,
                                                  name=str(checkpoint))
