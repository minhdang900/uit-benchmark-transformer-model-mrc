...
\vspace{0.6em}
\footnotesize Chênh lệch EM tổng (cặp): \textbf{\PairDiff}, CI 95\% theo bài [\PairArtLo; \PairArtHi], $p$ = \PairP\\[3pt]
\alert{$\Rightarrow$ Ở $\tau = 0$: không khác nhau có ý nghĩa.}\\[3pt]
\scriptsize Mốc: validation = public test VLSP 2021; baseline mBERT F1 \PubBaseFone, người F1 \PubHumanFone
\end{column}
\end{columns}
\src{results/eval\_*\_validation.json, results/diagnosis\_validation.json}
\end{frame}

\begin{frame}{Ở $\tau = 0$: hai hồ sơ kỹ năng \emph{ngược nhau}?}
\begin{columns}[T]
...
\small Hai chênh lệch phần lớn là \emph{một}: mô hình nào sẵn lòng trả lời hơn. Trên \BothN{} câu có đáp án
mà \emph{cả hai} cùng trả lời, khoảng cách ``đọc'' chỉ còn \textbf{\BothGap} điểm EM.
\end{block}
...
\midrule
\textcolor{xlmr}{\textbf{XLM-R}} & \ThrXlmrTau & \ThrXlmrFone & \ThrXlmrAbsRate\% \\
\textcolor{phobert}{\textbf{PhoBERT}} & \ThrPhobertTau & \ThrPhobertFone & \ThrPhobertAbsRate\% \\
\bottomrule
\end{tabular}

\vspace{0.4em}
\scriptsize Kiểm tra: dựng lại dự đoán từ điểm từng cửa sổ khớp \NVal/\NVal{} dự đoán đã lưu ở $\tau = 0$.
\end{column}
\begin{column}{0.46\textwidth}
\centering
\bignum{+\PairTDiff}{phobert}\\
\small EM cho PhoBERT, CI theo bài [\PairTArtLo; \PairTArtHi]\\[6pt]
\footnotesize Có đáp án: \textbf{+\PairTAnsDiff} · Không đáp án: \textbf{+\PairTImpDiff}
\end{column}
\end{columns}

\vspace{0.3em}
\begin{block}{``Hai hồ sơ ngược nhau'' là do hiệu chỉnh, không phải năng lực}
\small Ở ngưỡng đúng, PhoBERT tốt hơn trên \emph{cả hai} loại câu. Cần đọc cùng độ dao động theo seed (phụ lục).
\end{block}
...
$\bullet$ Khả năng phân biệt \textbf{còn}: điểm \code{[CLS]} chỉ dịch $\approx$ \ThrPhobertTau{} logit
\end{column}
\end{columns}
\src{results/probe\_resume.json, results/feature\_scan\_phobert.json, results/windows\_*\_dev.json}
\end{frame}

\begin{frame}{Huấn luyện lại ổn định (lr $10^{-5}$) --- có thay lần chạy chính?}
\small Quy tắc định trước: thay khi \textbf{ổn định} \emph{và} F1 toàn bộ dev \textbf{cao hơn}.\\[4pt]
\footnotesize
\renewcommand{\arraystretch}{1.2}
...
\vspace{0.6em}
\small \alert{Không thay}: lần chạy lr $3\cdot10^{-5}$ ``sụp đổ'' nhưng học vị trí đáp án tốt hơn, và ngưỡng đúng khôi phục khả năng từ chối.
...
\small
\begin{enumerate}
\item \textbf{So ở $\tau$ cố định là so hiệu chỉnh, không phải năng lực.} Ở $\tau = 0$: ``bằng nhau, hồ sơ ngược nhau''. Ở $\tau$ chọn trên dev: PhoBERT hơn \PairTDiff{} điểm EM trên \emph{cả hai} loại câu.
\item \textbf{PhoBERT ``không từ chối'' là một sự kiện khi huấn luyện}, tái hiện được, kèm gradient vọt; lr $10^{-5}$ tránh được.
\item \textbf{Tách từ không phải yếu tố quyết định}: chỉ \SegMisPct\% đáp án cắt ngang từ; khoảng cách ``đọc'' thuần \BothGap{} điểm.
...
\footnotesize 2 seed · 3 epoch, không tìm siêu tham số · \code{pyvi} thay RDRSegmenter · nguyên nhân sụp đổ mới xác định đến mức ``đủ để khắc phục'' · validation = public test VLSP (private test ẩn nhãn)
\end{block}
\end{frame}
...
PhoBERT lr $10^{-5}$ (ổn định): epoch \PhobertRunCBestEpoch.\\[3pt]
Lần chạy PhoBERT chính: quy tắc ``ổn định \emph{và} tốt hơn trên dev''.
\end{column}
\end{columns}
\end{frame}
