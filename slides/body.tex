...
$\bullet$ 2 seed (42, 13), không tìm siêu tham số
\end{column}
...
\begin{frame}{Bộ stress-test v2: kiểm định \emph{trước} khi dùng}
\begin{columns}[T]
\begin{column}{0.56\textwidth}
\includegraphics[width=\textwidth]{slide_stress_v2.png}
\end{column}
\begin{column}{0.42\textwidth}
\small
\bignum{\StressViol}{phobert}\\
vi phạm trên \StressN{} mục, \StressCtx{} ngữ cảnh --- \emph{mọi} mục chấm được

\vspace{0.3em}
\footnotesize
$\bullet$ Câu hỏi, đáp án vàng do \emph{người} gán; biến đổi chỉ theo luật suy ra được nhãn\\
$\bullet$ ``Luôn từ chối'' chỉ đạt \textbf{\AbstainStressEM} điểm\\[3pt]
\textbf{Kiểm bằng máy:} offset, đáp án đã mất thật, E5 hết dấu, không trùng train.

\vspace{0.2em}
\alert{$\Rightarrow$ Xếp hạng được, dùng để chẩn đoán.}
\end{column}
\end{columns}
\src{results/stress\_v2\_audit.json}
\end{frame}
...
\end{column}
\begin{column}{0.46\textwidth}
\small \textbf{EM stress-test v2 ($\tau$ dev)}\\[3pt]
\begin{tabular}{@{}l r@{}}
\toprule
...
\vspace{0.3em}
\alert{Vượt xa mức sàn}, nhưng bốn mô hình gần như ngang nhau.\\[2pt]
\footnotesize\textbf{Hỏng vì biến đổi} (đúng câu gốc, sai sau đó):
bỏ dấu phá \textbf{\XlmrEFiveBrokenPct\%} cặp (XLM-R), \textbf{\PhobertEFiveBrokenPct\%} (PhoBERT) ---
gấp $\geq$\,\StressEFiveMinRatio{} lần xoá câu chứa đáp án.
\end{column}
\end{columns}
\end{frame}
...
\item \textbf{Kiểm định công cụ đo trước khi dùng}: bộ cũ xếp ``không làm gì'' đứng đầu nên được dựng lại (v2: \StressViol{} vi phạm).
\end{enumerate}
