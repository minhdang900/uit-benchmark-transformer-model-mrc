...
\small Hai chênh lệch phần lớn là \emph{một}: ai sẵn lòng trả lời hơn. Trên câu có đáp án mà \emph{cả hai} cùng
trả lời, khoảng cách ``đọc'' chỉ \textbf{\BothGap} (seed 42) và \textbf{\BothSGap} (seed 13). Seed 13 \emph{không}
có hai hồ sơ ngược nhau: PhoBERT từ chối \PhobertThirteenAbsRate\%.
\end{block}
...
\footnotesize Ở ngưỡng đúng, PhoBERT tốt hơn về tổng ở \emph{cả hai} seed; dao động theo seed $<$ 1 điểm F1.
\end{block}
...
\item \textbf{Tách từ không phải yếu tố quyết định}: chỉ \SegMisPct\% đáp án cắt ngang từ; khoảng cách ``đọc'' thuần \BothSGap--\BothGap{} điểm.
