...
* **Có ``offset_mapping``.** Cần để map token span về ký tự trong context gốc.
  Không có nó, cách duy nhất lấy lại chuỗi là ``tokenizer.decode()`` — và decode
  **làm mất dấu tiếng Việt** (``"hoà"`` -> ``"hoa"``), phá cả EM lẫn bất biến
  substring. Model subword dùng fast tokenizer; PhoBERT (chỉ có slow tokenizer)
  được bọc bởi :class:`mrc.segmented_tokenizer.SegmentedTokenizer`, tự dựng offset
  từ bước tách từ. Xem :mod:`mrc.tokenization`.
...
class TransformerQA(TimedPredictorMixin):
    ...
    def __init__(
        ...
        from mrc.tokenization import load_tokenizer
        ...
        # Subword model: fast tokenizer. Word-level (PhoBERT): bọc để có offset —
        # xem mrc.tokenization. Model không có offset bị TỪ CHỐI, không chạy sai.
        tokenizer = load_tokenizer(model_name, word_segmented=word_segmented)
        self.word_segmented = word_segmented
    ...
    @classmethod
    def from_checkpoint(cls, checkpoint: str, **overrides) -> "TransformerQA":
        """Nạp checkpoint cùng đúng cấu hình tokenize đã dùng lúc huấn luyện."""
        from mrc.tokenization import read_tokenization_config

        config = read_tokenization_config(checkpoint)
